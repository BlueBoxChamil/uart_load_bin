...
#define SCALED( x )  ( (x) * ( 1L << shift ) - delta )
                      ...
                      const FT_Raster_Params*  params )
  {
    ...
    /* this version does not support monochrome rendering */
    if ( !( params->flags & FT_RASTER_FLAG_AA ) ) {
      ret = FT_THROW( Cannot_Render_Glyph );
      goto __exit;
    }

    if ( !outline ) {
      ret = FT_THROW( Invalid_Outline );
      goto __exit;
    }
  ...
  int
  gray_raster_dsp_render( FT_Raster                raster,
                      const FT_Raster_Params*  params )
  {
  ...
  int
  gray_raster_dsp_new( void*       memory,
                   FT_Raster*  araster )
  {
  ...
  void
  gray_raster_dsp_done( FT_Raster  raster )
  {
  ...
  void
  gray_raster_dsp_reset( FT_Raster       raster,
                     unsigned char*  pool_base,
                     unsigned long   pool_size )
 {
  ...
  int
  gray_raster_dsp_set_mode( FT_Raster      raster,
                        unsigned long  mode,
                        void*          args )
  {
  ...
  FT_DEFINE_RASTER_FUNCS(
    ...
    (FT_Raster_New_Func)     gray_raster_dsp_new,       /* raster_new      */
    (FT_Raster_Reset_Func)   gray_raster_dsp_reset,     /* raster_reset    */
    (FT_Raster_Set_Mode_Func)gray_raster_dsp_set_mode,  /* raster_set_mode */
    (FT_Raster_Render_Func)  gray_raster_dsp_render,    /* raster_render   */
    (FT_Raster_Done_Func)    gray_raster_dsp_done       /* raster_done     */
  )
#endif
