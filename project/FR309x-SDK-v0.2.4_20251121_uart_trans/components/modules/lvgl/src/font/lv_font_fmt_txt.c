...
const uint8_t * lv_font_get_bitmap_fmt_txt(const lv_font_t * font, uint32_t unicode_letter)
{
    ...
    if((uint32_t)fdsc->glyph_dsc & 0x08000000)
    {   
        //internal flash
    ...
    {
        lv_font_fmt_txt_glyph_dsc_t ext_gdsc;
        //extern flash
        extern_flash_read((uint8_t *)&ext_gdsc, ((uint32_t)fdsc->glyph_dsc + (16 * gid)), 16);
        gdsc = &ext_gdsc;
    }
                                   ...
                                   uint32_t unicode_letter_next)
{
    ...
    else
    {
        lv_font_fmt_txt_glyph_dsc_t ext_gdsc;
        //extern flash
        //printf("dsc : %x gid:%d \n",(uint32_t)fdsc->glyph_dsc,gid);
        extern_flash_read((uint8_t *)&ext_gdsc, ((uint32_t)fdsc->glyph_dsc + (16 * gid)), 16);
        gdsc = &ext_gdsc;
        //printf("dsc info-> id:%d,vw:%d,h:%d,w:%d,x:%d,y:%d \n",gdsc->bitmap_index,gdsc->adv_w,gdsc->box_h,gdsc->box_w,gdsc->ofs_x,gdsc->ofs_y);
    }
