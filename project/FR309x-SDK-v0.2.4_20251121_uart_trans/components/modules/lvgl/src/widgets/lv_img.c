...
void lv_img_set_src(lv_obj_t * obj, const void * src)
{
    ...
    uint8_t ext_flash_buffer[16];
    uint32_t img_data_address = (uint32_t)src;
    if((img_data_address & 0xF8000000) == 0)        //extern flash addres (0xF8000000  == 0)
    {
        if(ext_src.src != src)
        {
            //extern flash
            extern_flash_read((uint8_t *)&ext_src.dsc, (uint32_t)src, sizeof(lv_img_dsc_t));

        // lv_img_dsc_t *dsc = (lv_img_dsc_t *)ext_flash_buffer;
        // printf("scr :%x \n",src);
        // printf("dsc.header.h :%d \n",dsc->header.h);
        // printf("dsc.header.w :%d \n",dsc->header.w);
        // printf("dsc.header.reserved :%d \n",dsc->header.reserved);
        // printf("dsc.data:%x \n",dsc->data);
        // printf("dsc.data_size :%d \n",dsc->data_size);

            //extern flash start address is zero, src  will be masked as NULL.
            src = (const void *)(img_data_address | 0xC0000000);   //0xC0000000  as extern flash address sign

            ext_src.src = src;
            src_type = lv_img_src_get_type(&ext_src.dsc);
        }
        else
        {
            src_type = lv_img_src_get_type(&ext_src.dsc);
