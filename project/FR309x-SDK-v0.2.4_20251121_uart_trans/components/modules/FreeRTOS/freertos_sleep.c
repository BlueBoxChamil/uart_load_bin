...
__RAM_CODE __WEAK bool user_entry_before_sleep_check(void)
{
    return true;
}
    ...
    else
    {
        ...
        if ((ms < 5) 
            ...
            *(volatile uint32_t *)DEBUG_UART_BASE = 'a';
...
#if defined(CHIP_SEL_FR5099E)
            ///mcu pa8 ---> bt pa5, for wakeup
            SYSTEM->PortA_PullSelect |= GPIO_PIN_8;
            SYSTEM->PortA_PullEN |= GPIO_PIN_8;
            SYSTEM->PortA_H_FuncMux &= 0xfffffff0;             
#else
            ...
            if(user_entry_before_sleep_check() == false)
            {
                if(ms >= 5)
                    ms = 5;
            }
