 ...
 *          
 */
__RAM_CODE int System_SPLL_config(System_PLLConfig_t *PLLConfig, uint32_t fu32_timeout)
{
 ...
 *          
 */
__RAM_CODE int System_AUPLL_config(System_PLLConfig_t *PLLConfig, uint32_t fu32_timeout)
{
    if(trim_cp_get_version() == CP_VERSION_B)
    {
        if (PLLConfig->PowerEn)
        {
            ...
            if (PLLConfig->PLL_M * 2 > 65535) {
                ...
                SYSTEM->AUPLLConfig3.PLL_M = PLLConfig->PLL_M*2 - 65535;
            }
            else {
                SYSTEM->AUPLLConfig3.PLL_N = PLLConfig->PLL_N*2;
                SYSTEM->AUPLLConfig3.PLL_M = PLLConfig->PLL_M*2;
            }
            ...
            __SYSTEM_AUPLL_CLK_DIV2_ENABLE();
            System_AUPLLCLK = (PLLConfig->PLL_N*HSE_VALUE) + mul_mod(PLLConfig->PLL_M,HSE_VALUE,0xffff);
        }
        else
        {
...
{
    SYSTEM->AUPLLConfig3.PLL_M += 20; // d_sample = (20*24M/65535)/7/512
}
/*********************************************************************
 * @fn      System_AUPLL_Dec
 *
...
__RAM_CODE void system_xip_flash_init(System_XIPConfig_t *cfg, bool wake_up)
{
    ...
    GLOBAL_INT_DISABLE();
#if defined(CHIP_SEL_FR3066D) || defined(CHIP_SEL_FR3068E_V2) || defined(CHIP_SEL_FR3068E) || defined(CHIP_SEL_FR5098E) 
    if (cfg->DIV_SEL == QSPI_BAUDRATE_DIV_2) {
        while(1);
    }
    __SYSTEM_FR3068E_QSPI0_IO2_IO3_CONFIG();
#elif defined(CHIP_SEL_FR3092E) || defined(CHIP_SEL_FR3092E_V4) || defined(CHIP_SEL_FR5092E) || defined(CHIP_SEL_FR5099E)
    __SYSTEM_FR3092E_QSPI0_IO2_IO3_CONFIG();
#else
 ...
 * @fn      system_reset
 *
 * @brief   All reset(digital, analogy and pmu).
 */
